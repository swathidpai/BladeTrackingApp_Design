...
export function TeamFormPage() {
  ...
  const keyTaken = trimmedKey.length > 0 && teams.some((t) => t.key === trimmedKey && t.id !== editing?.id);
  const canSave = name.trim().length > 0 && color.length > 0 && !keyTaken;
            ...
            <div className="col-span-2">
              <Field label="Team name">
                <TextInput
                  ...
                  placeholder="e.g. Alpha Rope Access"
                />
              </Field>
            </div>
            <Field label="Colour">
      ...
      {deleteConfirm && editing && (
        ...
        >
          <p className="text-[13px] text-text-secondary">
            Work orders and jobs assigned to "{editing.name}" keep their work — they'll just fall back to
            "No team". This can't be undone.
          </p>
        </Modal>
      )}
    </div>
